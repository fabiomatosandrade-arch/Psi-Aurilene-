...
const Profile: React.FC<ProfileProps> = ({ user, onUpdate }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (userIndex !== -1) {
      ...
      setError('Erro ao localizar usuário.');
    }
  };
      ...
      <div className="space-y-6">
        <div className="bg-blue-50 p-6 rounded-[2rem] border border-blue-100 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold">Mantenha seus dados atualizados</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          {error && (
            <div className="bg-red-50 text-red-500 text-xs font-bold p-4 rounded-2xl text-center border border-red-100">
              {error}
            </div>
          )}

          {success && (
            <div className="bg-emerald-50 text-emerald-600 text-xs font-bold p-4 rounded-2xl text-center border border-emerald-100">
              {success}
            </div>
          )}

          <div className="grid grid-cols-1 gap-4 pt-4">
            <button type="submit" className="w-full brand-gradient text-white font-black py-5 rounded-2xl shadow-lg uppercase tracking-widest text-xs active:scale-95 transition-transform">
              Salvar Alterações
            </button>
            <button 
              type="button"
